...
#include "../../type.h" // AggType 定义于公共层 type.h
...
// 表达式基类：所有表达式节点的抽象接口。
struct Expr {
    enum class Type { COLUMN_REF, LITERAL, BINARY_OP, AGG_FUNC };
...
// 列引用，如 SELECT a / GROUP BY a 中的 a。
struct ColumnRefExpr : public Expr {
    std::string column_name;

    explicit ColumnRefExpr(std::string name) : Expr(Type::COLUMN_REF), column_name(std::move(name)) {}
...
struct AggFuncExpr : public Expr {
    AggType agg_type;
    ExprPtr arg; // 聚合参数；COUNT(*) 时为空

    AggFuncExpr(AggType agg_type, ExprPtr arg) : Expr(Type::AGG_FUNC), agg_type(agg_type), arg(std::move(arg)) {}
