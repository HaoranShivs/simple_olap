...
const char* BinaryOpName(PlanBinaryOp op) {
    ...
    case PlanBinaryOp::LE:
        return "<=";
    case PlanBinaryOp::AND:
        return "AND";
    case PlanBinaryOp::OR:
        return "OR";
    }
    return "?";
}
...
PlanExprPtr BuildPlanExpr(const BoundExpr& expr) {
    ...
    case BoundExpr::Type::BINARY_OP: {
        ...
        return std::make_unique<PlanBinaryExpr>(ToPlanBinaryOp(e.op), BuildPlanExpr(*e.left), BuildPlanExpr(*e.right),
                                                e.return_type);
    }
