...
BoundStatementPtr Binder::BindInsert(const InsertStatement& stmt) {
    // 1. 查表：从 Catalog 读取目录条目（纯内存，不涉及物理存储）
    const TableCatalogEntry* entry = catalog_.GetTable(stmt.table_name);
    if (entry == nullptr) {
    ...
    for (const auto& row : stmt.values) {
        ...
        for (size_t i = 0; i < row.size(); ++i) {
            ...
            if (!IsTypeCompatible(expected_type, actual_type)) {
                throw SemanticException("Type mismatch in INSERT for column index " + std::to_string(physical_col_idx));
            }
...
BoundStatementPtr Binder::BindSelect(const SelectStatement& stmt) {
    ...
    return std::make_unique<BoundSelectStatement>(context_->tables_[0].oid, std::move(bound_select_list),
...
std::unique_ptr<BoundExpr> Binder::BindBinaryOp(const BinaryOpExpr& expr) {
    ...
    default:
        dtype = (left->return_type == DataType::DOUBLE || right->return_type == DataType::DOUBLE) ? DataType::DOUBLE
                                                                                                  : left->return_type;
        break;
    }
...
std::unique_ptr<BoundExpr> Binder::BindAggFunc(const AggFuncExpr& expr) {
    if (!expr.arg) {
        ...
        return std::make_unique<BoundAggFunc>(AggType::COUNT, nullptr, DataType::INT64);
    }
    ...
    case AggType::COUNT:
        return std::make_unique<BoundAggFunc>(expr.agg_type, std::move(arg), DataType::INT64);
    case AggType::AVG:
        if (!IsNumeric(arg->return_type))
            throw SemanticException("AVG requires numeric input");
        return std::make_unique<BoundAggFunc>(expr.agg_type, std::move(arg), DataType::DOUBLE);
    case AggType::SUM:
        if (!IsNumeric(arg->return_type))
    ...
    case AggType::MAX: {
        ...
        return std::make_unique<BoundAggFunc>(expr.agg_type, std::move(arg), result_type);
    }
    default:
...
void Binder::BindTableRef(const std::string& table_name) {
    ...
    // 2. 从 Catalog 读取目录条目（纯内存，不涉及物理存储）
    const TableCatalogEntry* entry = catalog_.GetTable(table_name);
    if (entry == nullptr) {
