...
std::string LogicalScan::ToString(size_t indent) const {
    ...
    if (pushed_predicate_) {
        out << ", pushed=#" << pushed_predicate_->column_index << " "
            << CmpName(pushed_predicate_->op) << " "
            << LiteralToString(pushed_predicate_->value);
    }
    out << ")";
    return out.str();
}
...
std::string LogicalProject::ToString(size_t indent) const {
    ...
    for (size_t i = 0; i < outputs_.size(); ++i) {
        ...
        if (!outputs_[i].alias.empty()) out << " AS " << outputs_[i].alias;
    }
    out << ")\n" << child_->ToString(indent + 2);
    return out.str();
}
