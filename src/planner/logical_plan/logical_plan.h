...
public:
    explicit LogicalScan(uint32_t table_oid)
        : LogicalPlan(Type::SCAN), table_oid_(table_oid) {}

    uint32_t GetTableOid() const { return table_oid_; }
    const std::vector<uint32_t> &GetRequiredColumns() const { return required_columns_; }
    const std::optional<SimplePredicate> &GetPushedPredicate() const { return pushed_predicate_; }

    void SetRequiredColumns(std::vector<uint32_t> columns) { required_columns_ = std::move(columns); }
    void SetPushedPredicate(SimplePredicate predicate) { pushed_predicate_ = std::move(predicate); }
...
public:
    ...
    PlanExpr &MutablePredicate() { return *predicate_; }
    const LogicalPlan &GetChild() const { return *child_; }
    LogicalPlanPtr &MutableChild() { return child_; }
...
public:
    LogicalAggregate(std::vector<PlanExprPtr> group_by,
                     std::vector<NamedPlanExpr> outputs,
                     LogicalPlanPtr child)
        : LogicalPlan(Type::AGGREGATE),
...
public:
    LogicalInsert(uint32_t table_oid,
                  std::vector<uint32_t> target_columns,
                  std::vector<std::vector<PlanExprPtr>> rows)
        : LogicalPlan(Type::INSERT), table_oid_(table_oid),
