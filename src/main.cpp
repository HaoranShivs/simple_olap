...
uint64_t CountRows(Catalog& catalog, const std::string& table_name) {
    ...
    select.select_list.emplace_back(std::make_unique<ColumnRefExpr>(schema.columns[0].name));
...
void ExecuteSelect(Catalog& catalog, SelectStatement& select) {
    ...
    while (scan.Next(batch)) {
        ...
        const uint64_t valid_rows = select.where_clause ? static_cast<uint64_t>(batch.sel_vector.size()) : batch.size;
        total_rows += valid_rows;
    }

    std::cout << "SELECT ok: rows=" << total_rows << ", cols=" << select.select_list.size() << "\n";
}
...
void ExecuteInsert(Catalog& catalog, const InsertStatement& stmt) {
    ...
    if (stmt.columns.empty()) {
        ...
        for (size_t v = 0; v < stmt.columns.size(); ++v) {
            ...
            if (!found) {
                std::cout << "ERROR: column not found in table: " << stmt.columns[v] << "\n";
                return;
            }
        }
        ...
        for (size_t r = 0; r < nrows && vidx >= 0; ++r) {
            ...
            case DataType::INT32: {
                ...
                if (!std::holds_alternative<double>(literal.value)) {
                    std::cout << "ERROR: expected numeric value for column " << schema.columns[c].name << "\n";
                    return;
                }
            ...
            default:
                std::cout << "ERROR: unsupported column type for INSERT: " << schema.columns[c].name << "\n";
                return;
            }
        }
    ...
    std::cout << "INSERT ok: rows before=" << rows_before << ", rows after=" << rows_after << " (+" << nrows << ")\n";
    // 临时观察接口：展示 Insert 前后活跃 segment 的行数变化
    std::cout << "  active_segment rows: before=" << active_before << ", after=" << active_after << "\n";
}
