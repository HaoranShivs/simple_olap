...
HashAggregateOperator::GroupKey
HashAggregateOperator::EvalGroupKey(const VectorBatch &batch, uint32_t physical_row) const {
    GroupKey key;
...
void HashAggregateOperator::UpdateAggregate(const AggCallSpec &call,
                                             AggState &state,
...
ExecValue HashAggregateOperator::FinalizeAggregate(const AggCallSpec &call,
    ...
    case AggType::AVG:
        return static_cast<double>(state.count == 0
                                       ? 0.0L
                                       : state.sum / static_cast<long double>(state.count));
    case AggType::MIN:
