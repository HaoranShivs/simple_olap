...
namespace simple_olap
{
    ...
    void TableScan::Init()
    {
        ...
        // 2. 一次性绑定 request_：把列名解析为 column_id（按 select_list 顺序输出），
        ...
        {
            const ColumnId col_id = FindColumnId(schema, item.column_name);
            request_.target_list.push_back(SelectTarget{col_id, item.op});
        }

        // 3. 绑定 where 条件：列名 -> column_id，透传比较符与常量值。
        //    where 列若不在输出列中无需在此追加——
        //    StorageManager::GetVectorBatch 会在行级过滤前自行补齐并移除。
        if (selectstat_.where != nullptr)
        {
            auto expr = std::make_unique<ExprTarget>();
            expr->column_id = FindColumnId(schema, selectstat_.where->column_name);
            expr->op = selectstat_.where->op;
            expr->value = selectstat_.where->value; // int32_t
