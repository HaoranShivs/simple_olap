...
std::unique_ptr<Table> Table::Create(const TableMeta& metadata, const std::filesystem::path& root_dir) {
    ...
    auto storage = std::make_unique<StorageManager>(metadata.table_id, table_path, metadata);
...
std::unique_ptr<Table> Table::Open(TableId table_id, const std::filesystem::path& root_dir) {
    ...
    auto storage = std::make_unique<StorageManager>(table_id, table_path, metadata);
...
bool Table::GetVectorBatch(const ScanOptions& options, ScanCursor& cursor, VectorBatch& output) {
    // 委托给 StorageManager，由其跨 segment 推进游标并提取数据
    return storage_->GetVectorBatch(options, cursor, output);
}
