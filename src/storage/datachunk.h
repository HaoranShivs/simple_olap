...
class DataChunk {
  public:
    ...
    // 获取指定列的数据指针（已按 slice_start_ 行偏移）
    // 注意：偏移单位是"行"（slice_start_ * sizeof(T) 字节），
    // 因此 T 必须与该列的真实元素类型一致，不能用 uint8_t 代替。
    template <typename T> T* data(size_t col_idx) {
        return reinterpret_cast<T*>(chunks_[col_idx].get()) + slice_start_;
    }

    // 获取指定列的数据指针（const 版，已按 slice_start_ 行偏移）
    template <typename T> const T* data(size_t col_idx) const {
        return reinterpret_cast<const T*>(chunks_[col_idx].get()) + slice_start_;
    }

    // 获取指定列的原始字节指针（已按 slice_start_ 行偏移到正确字节位置）。
    // 与 data<T> 不同，本方法按 element_sizes_ 计算字节偏移，
    // 不依赖调用方提供正确的元素类型，供按字节搬运数据的存储层使用。
    const uint8_t* raw_data(size_t col_idx) const {
        return reinterpret_cast<const uint8_t*>(chunks_[col_idx].get()) + slice_start_ * element_sizes_[col_idx];
    }

    // 获取指定列的元素大小
    size_t element_size(size_t col_idx) const {
        return element_sizes_[col_idx];
    }
